...
| R8 | Export files capped at 10,000 rows → many files/day | ✅ | `ECOMMERCE_EXPORT_ROW_CAP`; sharded across Cloud Run tasks (D10). Measured: 31 files/day today, 304 at forecast |
| **Output** | | | |
...
| R11 | Output format suited to columnar storage | ✅ | Parquet + snappy, Hive-partitioned by date (D3). Measured 5.8× smaller than CSV |
...
| R13 | Scale 100k → 1M+ transactions/day | ✅ | DESIGN.md §6 — **measured**: 3.04M rows, 304 files, 45.0s, 93MB peak |
...
| R19 | Supporting python code / pseudocode | ✅ | `dbt/` and `deploy/` are runnable; `pipeline/` is pseudocode in real modules, with declarative parts literal. Built and run before reduction — see README |
...
| **Bonus** | | | |
| B1 | Testing methodology / unit tests | ✅ | `docs/testing.md` (methodology + what is deliberately untested), `tests/test_pipeline.py` (the plan), `dbt/tests/` (runnable). 129 unit + 50 data tests measured on the working version |
